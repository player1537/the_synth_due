...
with open('tables_due.h', 'w') as filehandle:
    ...
    for i in np.arange(120.0,132.0,(1.0/20.0)):
        top_octave.append((55.0 * np.power(2.0, (i+3.0)/12.0)) / 12.0)
...
0x0001,0x0001,0x0001,0x0001,0x0001,0x0001,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,\n\
    ...
    f = wave.open("Piano_C5.wav", 'r')
    x = f.readframes(-1)
    given_wave = np.fromstring(x, 'uint8')
    given_wave = [int(i) for i in given_wave]
    filehandle.write('\nconst uint8_t piano_C5_wave[] = {' + ', '.join(repr(x) for x in given_wave) + '};\n')

    f = wave.open("Piano_C6.wav", 'r')
    x = f.readframes(-1)
    given_wave = np.fromstring(x, 'uint8')
    given_wave = [int(i) for i in given_wave]
    filehandle.write('\nconst uint8_t piano_C6_wave[] = {' + ', '.join(repr(x) for x in given_wave) + '};\n')
...
\n\
    ...
    filehandle.write('struct Voice piano_C6 {\n\
	.max_length = 26569216,\n\
	.loop_point = 3507712,\n\
	.pitch_from_C5 = 12,\n\
\n\
